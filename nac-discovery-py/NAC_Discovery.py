...
def lambda_handler(event, context):
    ...
    merge_link = (
        '"https://' + link + '_opendistro/_security/api/rolesmapping/all_access"'
    )
    url = "https://" + link + "_opendistro/_security/api/rolesmapping/all_access/"

    headers = {"content-type": "application/json"}
    response = requests.put(
        url, auth=HTTPBasicAuth(username, password), headers=headers, data=role_data
    )
    print(response.text)
    ...
    for record in event["Records"]:
        ...
        data["root_handle"] = re.sub(
            "[!@#$%^&*()+?=,<>/.]", "-", secret_data_internal["root_handle"]
        ).lower()
        ...
        if secret_data_internal["web_access_appliance_address"] != "not_found":
            data["access_url"] = (
                "https://"
                + secret_data_internal["web_access_appliance_address"]
                + "/fs/view/"
                + data["volume_name"]
                + "/"
                + file_name
    ...
    try:
        ...
        for elem in es.cat.indices(format="json"):
            ...
            for i in resp["hits"]["hits"]:
                ...
                if (
                    idx_content == data["content"]
                    and idx_object_key == data["object_key"]
                ):
                    flag = 1
                    print(
                        "Indexing is doing when the idx_content and idx_object_key has matched",
                        resp,
                    )
        ...
        if flag == 0:
            ...
            resp = helpers.bulk(
                es, doc_list, index=data["root_handle"], doc_type="_doc"
            )
        ...
        logging.error(
            'ERROR: Unable to index line:"{0}"'.format(str(data["object_key"]))
        )
        print(e)
        return 1
